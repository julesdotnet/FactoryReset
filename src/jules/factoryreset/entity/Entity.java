...
public abstract class Entity {
	...
	public Node positionToNode() {
		...
		return new Node(Math.abs(x - BackgroundHandler.getOffsetX()) / tileSize,
				Math.abs(y - BackgroundHandler.getOffsetY()) / tileSize, 0, 0);
	}
	...
	protected void drawPathLines(Graphics g, Queue<int[]> queuedMovement) {
		...
		while (!usedQueue.isEmpty()) {
			...
			if (lastPoint != null) {
				g2.drawLine(lastPoint[0] - BackgroundHandler.getOffsetX() + BackgroundHandler.getTileSize() / 2,
